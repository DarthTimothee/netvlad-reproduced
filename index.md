...
Below are the resulting accuracies before and after training the networks. After our result is a copy of the result from the original paper.
...
While doing the experiments, we started out with a learning rate of `0.001` as is mentioned in the paper. But the results that came out were quite bad. The loss and accuracies are shown in the figure below (the upper loss curve is for test, the lower is for train. The accuracy curves are recall@N for N ranging from 1 through 25 as described in the section of evaluation).

![loss and accuracies for large learning rate](/fvlad-large-learn-rate.png)

We expected these results were due to the learning rate, causing the gradient descent to overshoot in the first few epoch, so we lowered the learning rate to `0.0001` which improved the performance quite a bit, as in shown in the figure below. However, the figure below shows quite a large gap between the train and test loss, which seems to indicate that the model is overfitting. This same behavior occurs in pretty much all of our experiments. We have tried tuning the hyper parameters, especially setting the weight decay to 40 times as large as proposed by the paper, and we tried different optimizers, but these problems remain. Likely, this can be solved by, possibly a combination of, further tuning the hyperparamters and training a on a larger training set. Sadly we did not have the time and available resources to do so.
